...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["flare-2.json",new URL("/project/circlemap/json/",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`<img src="/img/kprime-logo-bw.png" width="10%">`
)});
  main.variable(observer("chart")).define("chart", ["pack","data","d3","width","height","color"], function(pack,data,d3,width,height,color)
{
  ...
  const node = svg.append("g")
    ...
    .join("circle")
      .attr("fill", d => d.children ? color(d.depth) : d.data.type === 'goal' ? "#FFB"  : d.data.type === 'term' ? "#BBF" : "#BFB")
...
d3.scaleLinear()
    .domain([0, 5])
    .range(["rgb(238,238,238)", "rgb(151,0,0)"])
