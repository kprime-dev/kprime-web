 ...
 */
    ...
    internal fun demo1_5_when(parser: CmdParser, context: CmdContext) {
        ...
        checkOk(parser.parse("add-cs-inclusion table7:SSN-->table3:SSN", context).first)
        checkOk(parser.parse("add-cs-inclusion table7:SSN-->table1:SSN", context).first)
        checkOk(parser.parse("add-cs-double-inc table7:DepName<->table8:DepName", context).first)

        checkOk(parser.parse("del-cs-table table6", context).first)
        checkOk(parser.parse("del-cs-mapping table6", context).first)

        checkOk(parser.parse("add-cs-mapping table7 select SSN,DepName from table0 where DepName is not null and DepAddress is not null", context).first)
        checkOk(parser.parse("add-cs-mapping table8 select distinct DepName,DepAddress from table0 where DepName is not null and DepAddress is not null", context).first)
    }
    ...
    fun test_changeset_demo1_5_when() {
        ...
        // then
        assertEquals(11 + 1, context.env.changeSet.size()) // 9+1 del-table-4 imply del-constraints-table-4
    ...
    fun test_changeset_demo1_5_chageset_sql() {
        ...
        assertEquals("CREATE TABLE public.table7 AS\n" +
                "SELECT SSN,DepName\n" +
                "FROM   table0\n" +
                "WHERE DepName is not null and DepAddress is not null", sql[0])
        assertEquals("CREATE TABLE public.table8 AS\n" +
                "SELECT distinct DepName,DepAddress\n" +
                "FROM   table0\n" +
                "WHERE DepName is not null and DepAddress is not null", sql[1])
        assertEquals("ALTER TABLE table7 ALTER COLUMN SSN Varchar NOT NULL", sql[2])// double-inc
        assertEquals("ALTER TABLE table7 ADD PRIMARY KEY (SSN)", sql[3])
        assertEquals("ALTER TABLE table8 ALTER COLUMN DepName Varchar NOT NULL", sql[4])// double-inc
        assertEquals("ALTER TABLE table8 ADD PRIMARY KEY (DepName)", sql[5])
    }
