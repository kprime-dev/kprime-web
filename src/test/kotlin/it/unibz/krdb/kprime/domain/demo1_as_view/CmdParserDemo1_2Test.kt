...
class CmdParserDemo1_2Test: TraceParserTest() {
    ...
    internal fun demo1_2_when(parser: CmdParser, context: CmdContext) {
        ...
        checkOk(parser.parse("add-cs-double-inc table4:SSN<->table4:SSN", context).first)
        checkOk(parser.parse("del-cs-table table2", context).first)
        checkOk(parser.parse("del-cs-mapping table2", context).first)
        checkOk(parser.parse("add-cs-mapping table3 select SSN,Name from table0", context).first)
        checkOk(parser.parse("add-cs-mapping table4 select SSN,DepName,DepAddress from table0", context).first)
    }
    ...
    fun test_changeset_demo1_2_when() {
        ...
        // then
        assertEquals(11 + 1, context.env.changeSet.size())// 11+1 drop table => drop constraints
    ...
    fun test_changeset_demo1_2_chageset_sql() {
        ...
        // then
        assertEquals(11, sql.size)
        assertEquals("CREATE TABLE IF NOT EXISTS table3 (  SSN varchar(64) ,Name varchar(64));", sql[0])
        assertEquals("CREATE TABLE IF NOT EXISTS table4 (  SSN varchar(64) ,DepName varchar(64) ,DepAddress varchar(64));", sql[1])
        assertEquals("CREATE OR REPLACE VIEW public.table3 AS\n" +
                "SELECT SSN,Name\n" +
                "FROM   table0" , sql[2])
        assertEquals("CREATE OR REPLACE VIEW public.table4 AS\n" +
                "SELECT SSN,DepName,DepAddress\n" +
                "FROM   table0", sql[3])
        assertEquals("ALTER TABLE table1 ALTER COLUMN SSN Varchar NOT NULL", sql[4])
        assertEquals("ALTER TABLE table1 ALTER COLUMN Phone Varchar NOT NULL", sql[5])
        assertEquals("ALTER TABLE table1 ADD PRIMARY KEY (SSN,Phone)", sql[6])
        assertEquals("ALTER TABLE table3 ALTER COLUMN SSN Varchar NOT NULL", sql[7])// double-inc
        assertEquals("ALTER TABLE table3 ADD PRIMARY KEY (SSN)", sql[8])
        assertEquals("ALTER TABLE table4 ALTER COLUMN SSN Varchar NOT NULL", sql[9])
        assertEquals("ALTER TABLE table4 ADD PRIMARY KEY (SSN)", sql[10])
    }
