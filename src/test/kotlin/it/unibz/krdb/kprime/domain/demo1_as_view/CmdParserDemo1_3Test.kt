...
class CmdParserDemo1_3Test: TraceParserTest() {
    ...
    internal fun demo1_3_when(parser: CmdParser, context: CmdContext) {
        ...
        checkOk(parser.parse("add-cs-double-inc table3:SSN<->table5:SSN", context).first)
        checkOk(parser.parse("add-cs-double-inc table3:SSN<->table6:SSN", context).first)
        checkOk(parser.parse("add-cs-double-inc table1:SSN<->table5:SSN", context).first)
        checkOk(parser.parse("add-cs-double-inc table1:SSN<->table6:SSN", context).first)
    ...
    fun test_changeset_demo1_3_chageset_sql() {
        ...
        assertEquals("CREATE TABLE IF NOT EXISTS table5 (  SSN varchar(64));", sql[0])
        assertEquals("CREATE TABLE IF NOT EXISTS table6 (  SSN varchar(64) ,DepName varchar(64) ,DepAddress varchar(64));", sql[1])
        assertEquals("CREATE OR REPLACE VIEW public.table5 AS\n" +
                "SELECT SSN\n" +
                "FROM   table0\n" +
                "WHERE DepName is null and DepAddress is null", sql[2])
        assertEquals("CREATE OR REPLACE VIEW public.table6 AS\n" +
                "SELECT SSN,DepName,DepAddress\n" +
                "FROM   table0\n" +
                "WHERE DepName is not null and DepAddress is not null", sql[3])
        assertEquals("ALTER TABLE table5 ALTER COLUMN SSN Varchar NOT NULL", sql[4])// double-inc
        assertEquals("ALTER TABLE table5 ADD PRIMARY KEY (SSN)", sql[5])
        assertEquals("ALTER TABLE table6 ALTER COLUMN SSN Varchar NOT NULL", sql[6])// double-inc
        assertEquals("ALTER TABLE table6 ADD PRIMARY KEY (SSN)", sql[7])
    }
