...
class CmdParserDemo1_1Test: TraceParserTest() {
    ...
    fun test_changeset_demo1_1_when() {
        ...
        // then
        assertEquals(6 + 1, context.env.changeSet.size())// 7+1 drop-table => drop-constraint
    ...
    fun test_changeset_demo1_1_chageset_sql() {
        ...
        assertEquals("CREATE TABLE public.table1 AS\n" +
                "SELECT SSN,Phone\n" +
                "FROM   table0", sql[0])
        assertEquals("CREATE TABLE public.table2 AS\n" +
